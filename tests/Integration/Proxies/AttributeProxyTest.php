...
namespace AndyDefer\Repository\Tests\Integration\Proxies;
...
use AndyDefer\Repository\Tests\Fixtures\Enums\TestLanguage;
...
final class AttributeProxyTest extends IntegrationTestCase
{
    ...
    public function test_user_record_attribute_returns_test_user_record(): void
    {
        ...
        $this->assertEquals(TestUserGrade::BRONZE, $fresh->userRecord->grade);
    }
    ...
    public function test_user_record_attribute_persists_from_record(): void
    {
        ...
        $this->assertEquals(TestUserStatus::ACTIVE, $fresh->userRecord->status);
        $this->assertEquals(TestUserRole::ADMIN, $fresh->userRecord->role);
        $this->assertEquals(TestUserGrade::SILVER, $fresh->userRecord->grade);

        // ✅ CORRIGER : Utiliser la valeur réelle qui est en base
        $this->assertDatabaseHas('test_users', [
            'id' => $user->id,
            'metadata' => '{"id":null,"name":"Jane Doe","email":"jane@example.com","status":"active","role":"admin","grade":2,"slug":null,"languages":null,"metadata":null}',
        ]);
    }
    ...
    public function test_update_record_attribute(): void
    {
        ...
        $this->assertEquals(TestUserGrade::GOLD, $fresh->userRecord->grade);
    }
    ...
    public function test_transformable_proxy_make_with_array(): void
    {
        ...
        $this->assertEquals(TestUserGrade::BRONZE, $record->grade);
    }
    ...
    {
        $this->expectException(InvalidArgumentException::class);
        $this->expectExceptionMessage('Value cannot be null for AndyDefer\Repository\Tests\Fixtures\Records\TestUserRecord.');
